...
{
	PkModel *model;   /* Data model for storing statistics. */
...
static GQuark size_quark;
static GQuark resident_quark;
static GQuark share_quark;
static GQuark text_quark;
...
ppg_memory_instrument_process_cb (PpgVisualizerEntry  *entry,
                                  PpgMemoryInstrument *instrument,
                                  GError              **error)
{
	...
	line = g_object_new(PPG_TYPE_RENDERER_LINE, NULL);
	visualizer = g_object_new(PPG_TYPE_VISUALIZER_SIMPLE,
	                          "name", "process",
	                          "renderer", line,
	                          "title", _("Process Memory Usage"),
	                          NULL);
	ppg_renderer_line_append(line, priv->model, size_quark);
	ppg_renderer_line_append(line, priv->model, resident_quark);
	ppg_renderer_line_append(line, priv->model, share_quark);
	ppg_renderer_line_append(line, priv->model, text_quark);
	ppg_renderer_line_append(line, priv->model, data_quark);
...
ppg_memory_instrument_class_init (PpgMemoryInstrumentClass *klass)
{
	...
	size_quark = g_quark_from_static_string("Size");
	resident_quark = g_quark_from_static_string("Resident");
	share_quark = g_quark_from_static_string("Share");
	text_quark = g_quark_from_static_string("Text");
	data_quark = g_quark_from_static_string("Data");
}
